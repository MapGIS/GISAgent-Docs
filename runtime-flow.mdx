...
- `qoderclicn --acp`（`@qodercn-ai/qoderclicn`）不会仅凭 `/workspace/skills` 自动发现 `skillpacks`
- 当前有效方式是在 runtime 启动时把 `skillpacks` 增量平铺到 `~/.qoder-cn/skills`
- 这是保留原有 `.qoder-cn` 内容基础上的追加，不是覆盖
